...
export class VercelBlobStorageAdapter implements StorageAdapter {
  ...
  async loadWorkspace(key: string): Promise<StorageLoadResult | null> {
    ...
    if (!response.ok) {
      throw new Error(`Failed to load workspace state for ${key}: ${response.status}`);
    }
  ...
  #resolveStateBlob(listing: ListBlobResult, statePath: string): { url: string } | null {
  ...
  async saveWorkspace(key: string, payload: WorkspacePayload): Promise<StorageSaveResult> {
