...
describe("Workspace hydration", () => {
  it("hydrates stored files into mount path", async () => {
    ...
    expect(sandbox.writtenFiles[0]?.path).toBe("/vercel/sandbox/workspace/src/index.ts");
    expect(new TextDecoder().decode(sandbox.writtenFiles[0]?.content)).toBe("console.log(1)");
