...
describe("VercelBlobStorageAdapter", () => {
  it("persists and loads manifest + files via blob state", async () => {
    const { createVercelBlobStorageAdapter } = await import("../adapters/vercel-blob");
    ...
    expect(loaded?.manifest.updatedAt).toEqual(new Date("2026-03-17T00:00:00Z"));
    ...
    expect(new TextDecoder().decode(loaded?.files[0]!.content)).toBe("hello\nfrom blob");
  });
