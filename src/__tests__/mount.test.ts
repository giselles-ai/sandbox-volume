...
import type { StorageAdapter, StorageLoadResult, StorageLock } from "../adapters/types";
...
class StaleReleaseAdapter implements StorageAdapter {
  ...
  async releaseLock(): Promise<void> {
    this.events.push(`release:${this.lock.leaseId}`);
    throw new WorkspaceLockStaleError(this.lock.key, this.lock.mode, this.lock.leaseId);
  }
}
...
describe("mount", () => {
  it("acquires and releases lock in success path with implicit commit", async () => {
    ...
    const value = await volume.mount(sandbox as unknown as Sandbox, async (txSandbox) => {
  ...
  it("propagates conflict as WorkspaceLockConflictError", async () => {
    ...
    await expect(volume.begin(sandbox as unknown as Sandbox)).rejects.toBeInstanceOf(
