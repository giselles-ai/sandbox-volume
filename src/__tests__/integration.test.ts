...
describe("memory adapter integration", () => {
  it("persists files across transactions and preserves deletions", async () => {
    ...
    expect(secondSandbox.writtenFiles[0]?.path).toBe("/vercel/sandbox/workspace/src/index.ts");
  ...
  it("persists only filtered paths across multiple transactions", async () => {
    ...
    secondSandbox.setFileState({
      "/vercel/sandbox/workspace/package.json": '{"name":"include-exclude","version":1}',
    ...
    const secondCommit = await tx2.commit();
    expect(secondCommit.committed).toBe(true);
    expect(secondCommit.diff.changes.map((change) => change.path).sort()).toEqual([
      "package.json",
      "src/index.ts",
      "src/new.ts",
    ]);
    expect(
      secondCommit.diff.changes.some((change) => change.path.startsWith("src/generated/")),
    ).toBe(false);
  ...
  it("keeps out-of-scope historical entries until a scoped rewrite/resync", async () => {
    ...
    await sourceVolume.mount(bootstrapSandbox as unknown as Sandbox, async () => {
    ...
    const scopedTx = await scopedVolume.begin(scopedSandbox as unknown as Sandbox);
    ...
    const rewriteResult = await scopedVolume.rewrite(rewrittenSandbox as unknown as Sandbox);
    ...
    const resyncResult = await scopedVolume.resync(resyncSandbox as unknown as Sandbox);
  ...
  it("is fully usable through package public exports", async () => {
    ...
    const explicitError = new WorkspaceLockConflictError("repo/public", "exclusive");
    ...
    expect(verifySandbox.writtenFiles.map((file) => file.path).sort()).toEqual(
      ["/vercel/sandbox/workspace/package.json", "/vercel/sandbox/workspace/src/index.ts"].sort(),
    );
    ...
    const commitAllResult = await volume.commitAll(commitOnlySandbox as unknown as Sandbox);
