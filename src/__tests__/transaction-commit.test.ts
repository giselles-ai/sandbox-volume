...
type CommandHandler = (command: string, args: string[]) => Promise<MockCommandResult>;
...
class TransactionLockAdapter implements StorageAdapter {
  ...
  async releaseLock(): Promise<void> {
    ...
    if (this.releaseBehavior === "stale") {
      throw new WorkspaceLockStaleError(this.key, "exclusive", this.lock.leaseId);
    }
...
describe("workspace transaction commit", () => {
  it("commits modified files with update diff", async () => {
    ...
    const initialFiles = [createWorkspaceFile("src/index.ts", "console.log(1)")];
  ...
  it("ignores excluded files during scan and commit payload", async () => {
    ...
    expect(saveCall?.payload.files.some((file) => file.path === "coverage/out.json")).toBe(false);
    await tx.close();
  });
  ...
  it("does not emit deletes for newly excluded historical files", async () => {
    ...
    expect(result.diff.changes.every((change) => change.path !== "dist/out.js")).toBe(true);
    expect(adapter.saveCalls).toHaveLength(0);
    await tx.close();
  });
  ...
  it("returns explicit lock release error from transaction close", async () => {
    const sandbox = createMockSandbox();
    const adapter = new TransactionLockAdapter("repo/tx-release-fail", "generic");
    const volume = await SandboxVolume.create({
      adapter,
