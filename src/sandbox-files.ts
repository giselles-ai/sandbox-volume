...
async function scanWithFind(sandbox: Sandbox, mountPath: string): Promise<string[]> {
  ...
  if (result.exitCode !== 0) {
    ...
    const message =
      output.trim().length > 0 ? ` ${output.trim()}` : ` (exitCode=${result.exitCode})`;
...
async function scanWithNode(sandbox: Sandbox, mountPath: string): Promise<string[]> {
  const result = await sandbox.runCommand("node", ["-e", NODE_SCAN_SCRIPT, mountPath]);
  if (result.exitCode !== 0) {
    const output = await result.stdout();
    const message =
      output.trim().length > 0 ? ` ${output.trim()}` : ` (exitCode=${result.exitCode})`;
...
function toRelativeWorkspacePath(mountPath: string, absolutePath: string): string {
  const normalizedMountPath = normalizeMountPath(mountPath);
...
export async function collectWorkspaceFiles(
  ...
  const normalizedMountPath = normalizeMountPath(mountPath);
  const fileList =
    filePaths.length > 0 ? filePaths : await scanWorkspaceFilePaths(sandbox, normalizedMountPath);
