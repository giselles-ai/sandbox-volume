...
export class WorkspaceTransaction implements WorkspaceTransactionHandle {
  ...
  async open(): Promise<void> {
    ...
    if (lockMode !== "none") {
      if (!this.#adapter.acquireLock || !this.#adapter.releaseLock) {
        throw new Error("StorageAdapter requires acquireLock and releaseLock for locking.");
      }
    ...
    try {
      ...
      if (loaded) {
        ...
        const managedPaths = new Set(filteredManifest.paths.map((entry) => entry.path));
        this.#baselineManifest = filteredManifest;
        this.#baselineFiles = loaded.files.filter((file) => managedPaths.has(file.path));
      }
      await hydrateWorkspaceFiles(
        this.#sandbox,
        this.#mountPath,
        loaded?.files.filter((file) =>
          this.#pathRules ? filterPathsByRules([file.path], this.#pathRules).length > 0 : true,
        ) ?? [],
      );
      this.#opened = true;
    } catch (error) {
  ...
  async #acquireLock(lockMode: LockMode): Promise<StorageLock> {
    if (!this.#adapter.acquireLock) {
      throw new Error("StorageAdapter requires acquireLock to perform locking.");
    }
    ...
    try {
      ...
      if (error instanceof Error && /(already|held|conflict|exists)/i.test(error.message)) {
  ...
  async #commitWithOptions(options: CommitOptions = {}): Promise<WorkspaceCommitResult> {
    await this.open();
  ...
  async #scan(): Promise<WorkspaceFileEntry[]> {
    const paths = await scanWorkspaceFilePaths(this.#sandbox, this.#mountPath, this.#pathRules);
  ...
  async #releaseLock(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof WorkspaceLockReleaseError || error instanceof WorkspaceLockError) {
        throw new WorkspaceLockReleaseError(lock.key, lock.mode, lock.leaseId, error);
      }
